...
def process_posts(raw_file_path, processed_file_path=None):
    with open(raw_file_path, encoding='utf-8') as file:
        ...
        for post in posts:
            ...
            # Add hashtags to the post text
            hashtags = generate_hashtags(metadata['tags'])
            post_with_metadata['text'] += f"\n\n{hashtags}"
...
def generate_hashtags(tags):
    """
    Generates a string of hashtags from the given tags.
    Example: ['motivation', 'self-improvement'] -> '#Motivation #SelfImprovement'
    """
    hashtags = ' '.join(f"#{tag.title().replace(' ', '')}" for tag in tags)
    return hashtags
